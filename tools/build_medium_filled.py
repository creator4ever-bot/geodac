#!/usr/bin/env python3
# Build Medium filled.json: дополняет недостающие Sun..Mars к наталу по 0/60/90/120/180 (topo), без дублей
import os, sys, json, re, datetime as dt
from zoneinfo import ZoneInfo
try:
    import swisseph as swe
except Exception:
    print("ERR: pip install pyswisseph в astroenv", file=sys.stderr); sys.exit(2)
...
if __name__=="__main__":
    import argparse
    ap=argparse.ArgumentParser(description="Build Medium filled (offline topo)")
    ...
    ap.add_argument("--orb_conj", type=float, default=1.5); ap.add_argument("--orb_other", type=float, default=1.0)
    ...
    jd0  = jd_iso(args.FROM); jd1 = jd_iso(args.TO)
    step = args.step_min/1440.0
    ...
    # текущие пики (чтобы не дублировать)
    ...
    for e in EV:
        tr=e.get('transit'); tar=e.get('target'); a=next((k for k,v in ASP.items() if v==e.get('aspect')),None)
        if tr and tar and a is not None:
            seen.add((tr,tar,a))
            pkmap.setdefault((tr,tar,a),[]).append(parse_loc(e.get('peak') or e.get('start')))
    need=[]
    state={}
    jd=jd0
    while jd<=jd1+1e-9:
                ...
                for a in aspects:
                    ...
                    if inside:
                        ...
                        if d>orb:
                            ...
                            if not any(abs((pk_dt-x).total_seconds())<=21600 for x in pkmap.get(key,[])):
                                # найти границы
                                ...
                                ev={"transit":tr,"target":tar,"aspect":ASP[a],"aspect_deg":a,
                                    "orb_peak_deg":round(best_d,3),
                                    "houses":{"tr":None,"nat":None},
    ...
    out = EV + need
    # оформить newstyle и «Проф. разбор»
    for e in out:
        tr=e.get("transit"); tar=e.get("target"); ad=e.get("aspect_deg"); asp=e.get("aspect")
        e["summary"]=f"{EMO.get(tr,tr)} {asp} {EMO.get(tar,tar)}"
        ...
        # final guard
        if e["end"]<=e["start"]:
            t0=parse_loc(e["start"]); e["end"]=(t0+dt.timedelta(minutes=1)).strftime("%Y-%m-%d %H:%M")
    json.dump(out, open(args.json_out,'w',encoding='utf-8'), ensure_ascii=False, indent=2)
    print("added events:", len(need), "| total:", len(out))
