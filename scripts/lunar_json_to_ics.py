...
def asp_key(e):
  a=str(e.get('aspect','') or '').strip().lower()
  for k,v in SYM.items():
    if k in a or k in e.get('summary',''): return v
  return 'conj'
def orb_minutes(e, ak):
  # peak может быть строкой/числом/объектом — берём аккуратно
  pk=e.get('peak',{})
  orb=None
...
def main():
  if len(sys.argv)<3: print('usage: lunar_json_to_ics.py in.json out.ics [days=14]', file=sys.stderr); sys.exit(2)
  ...
  for e in items:
    ...
    clamp = s + timedelta(minutes=orb_minutes(e, ak))
    t = clamp
    if e.get('end'):
